...
import { FileText, Upload, Sparkles, AlertCircle, CheckCircle, Loader, Trash2, X } from 'lucide-react';
...
const LabReports = () => {
  ...
  const deleteReport = async (reportId, reportName) => {
    ...
    try {
      ...
      if (response.ok) {
        await fetchReports();
        if (analysis && analysis.id === reportId) {
          setAnalysis(null);
        }
  ...
  const viewAnalysis = async (reportId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8000/nutrition/lab-reports/${reportId}/analysis`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
          ...
          >
            <Card className="analysis-card">
              <div className="analysis-header">
                <h2>
                  <Sparkles size={24} />
                  AI Analysis: {analysis.report_name}
                </h2>
                <button 
                  className="close-btn"
                ...
                <>
                  {analysis.ai_summary && (
                    <div className="analysis-section">
                      <h3>Summary</h3>
                      <p className="summary-text">{analysis.ai_summary}</p>
                    </div>
                  )}
                  ...
                  <div className="disclaimer">
                    <AlertCircle size={16} />
                    <p>This AI analysis is for informational purposes only. Always consult with your healthcare provider for medical advice.</p>
                  </div>
                </>
              )}
            </Card>
